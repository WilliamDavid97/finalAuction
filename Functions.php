...
public function registerUser($name, $email,$phNo,$nrc, $password,$address,$dob,$gender,$cardNo) {

	$db = $this -> db;

	if (!empty($name) && !empty($email) && !empty($phNo)  && !empty($nrc) && !empty($password) && !empty($address) && !empty($dob) && !empty($gender) && !empty($cardNo)) {
  		...
  		if ($db -> checkUserExist($email)) {
  			...
  			$result = $db -> insertData($name, $email,$phNo,$nrc, $password,$address,$dob,$gender,$cardNo);
...
public function sendEmail($email,$temp_password){
  ...
  $mail->SMTPAuth = true;
  $mail->Username = 'jokerboy97.jk@gmail.com';
  $mail->Password = 'justinjoker97';
  $mail->SMTPSecure = 'ssl';
  $mail->Port = 465;
  $mail->setFrom('jokerboy97.jk@gmail.com', 'Joker');
  $mail->addAddress($email);
  $mail->addReplyTo('jokerboy97.jk@gmail.com');
  $mail->WordWrap = 50;
  $mail->isHTML(true);
 
  $mail->Subject = 'Password Reset Request';
  $mail->Body    = 'Hi,<br><br> Your password reset code is <b>'.$temp_password.'</b> . This code expires in 120 seconds. Enter this code within 120 seconds to reset your password.<br><br>Thanks,<br>Learn2Crack.<b>Reference By A Than Lay</b>';
